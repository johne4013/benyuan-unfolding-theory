...
## 任务工具示例 / Task Tool Examples

理论与治理框架最终要落到**可执行的任务工具**。仓库内附带可直接运行的示例：

- `admission/` — 心脏外科主诉/现病史/查体快速生成器（医生勾选要素即时成文）
- `cardiac-discharge/` — 心脏外科出院告知单个性化生成系统

它们演示了"理论核心 → 治理边界 → 任务执行"的落地路径：在明确的约束与边界下，用工具提升真实任务的效率。
